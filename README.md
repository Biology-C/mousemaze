...
- 18 關漸進式內容，含 6 關教學
- 獨立「小小探險家」模式：20 關線索迷宮，適合約 5～7 歲練習數列、顏色觀察及 11～20 的十位／個位
- 純前端、零依賴、開啟瀏覽器即可遊玩
- 支援繁中 / 英文、桌機鍵盤與手機搖桿操作
...
- **20 關線索學習迷宮**：分成 5 個章節，交錯練習 `+1`、雙數、`+5`、11～20、12／20 對照、顏色順序與數字＋顏色雙線索。藍色長盒代表十、橘色小點代表一；答錯不扣分，錯誤候選會留下測試記號，完成線索後才會開啟出口。
- **適合手機的慢速單格操作**：教育模式速度為冒險模式的一半；每次撥動搖桿只走一格，回到中央後才能再次移動，降低走過頭的挫折。
...
│  ├─ game_education.js# 教育模式流程
│  ├─ game_progress.js# 存檔 / 結算 / 排行榜流程
│  ├─ education.js    # 數字 / 顏色 / 位值線索迷宮引擎
│  ├─ education_content.js # 20 關教育課程資料
│  ├─ ui.js           # UI 互動與選單
│  ├─ ui_content.js   # 多語系文案
│  ├─ maze.js         # 迷宮生成與路徑搜尋
│  ├─ player.js       # 玩家控制與技能
